...
(
	...
	@Salt VARBINARY (16) --Unique random binary value added to strengthen the hash
)
RETURNS VARBINARY(64)
AS
BEGIN

	--Ensuring 2 identical passwords produce different hashes by combining salt and password
	RETURN HASHBYTES ('SHA2_512', CAST(@Salt AS VARCHAR(50)) + @Password)
END
GO
--/////////////////////////////////////////////////////////////--
...
CREATE TABLE Admins (
    ...
    Surname NVARCHAR(50),
    PasswordHash VARBINARY(64) NOT NULL, --Stores the hashed password, never plain text
    PasswordSalt VARBINARY(16) NOT NULL DEFAULT CRYPT_GEN_RANDOM(16), --Auto-generates a random salt
...
--Admin Data--
-- Insert Sarah Miller - SuperUser
DECLARE @SaltSarah VARBINARY(16) = CRYPT_GEN_RANDOM(16); -- Randomly generates Sarah's unique salt
INSERT INTO Admins (UserName, FirstName, Surname, PasswordHash, PasswordSalt, AdminRole)
VALUES (
'superadmin',
N'Sarah',
N'Miller',
dbo.fn_HashPassword('Sarah@Secure1!', @SaltSarah), -- Hash generated from random salt + password
@SaltSarah, -- Same salt saved for future login verification
'SuperUser'
);
GO

-- Insert Tom Wilson - Sales
DECLARE @SaltTom VARBINARY(16) = CRYPT_GEN_RANDOM(16);
INSERT INTO Admins (UserName, FirstName, Surname, PasswordHash, PasswordSalt, AdminRole)
VALUES (
'ticket_mgr',
N'Tom',
N'Wilson',
dbo.fn_HashPassword('Tom@Secure2!', @SaltTom), 
@SaltTom,
'Sales'
);
GO

-- Insert Anna Lee - Operator
DECLARE @SaltAnna VARBINARY(16) = CRYPT_GEN_RANDOM(16); 
INSERT INTO Admins (UserName, FirstName, Surname, PasswordHash, PasswordSalt, AdminRole)
VALUES (
'match_coord',
N'Anna',
N'Lee',
dbo.fn_HashPassword('Anna@Secure3!', @SaltAnna), 
@SaltAnna,
'Operator'
);
GO

-- Verify hashes are full SHA2_512 binary values and salts are random binary values
SELECT AdminID, UserName, FirstName, Surname, PasswordSalt, PasswordHash, AdminRole
FROM Admins;
GO
...
-- Scalar function: Team points calculator
-- Calculates total points for a team based on match results (Win=3, Draw=1, Loss=0)
CREATE FUNCTION dbo.GetTeamPoints (@TeamID INT)
RETURNS INT
AS
BEGIN
	DECLARE @Points INT;

	SELECT @Points = COALESCE(SUM(
		CASE
--Win: 3 points
			WHEN (m.Team1ID = @TeamID AND m.Team1Score > m.Team2Score)
				OR (m.Team2ID = @TeamID AND m.Team2Score > m.Team1Score)
			THEN 3

--Draw: 1 point
			WHEN m.Team1Score = m.Team2Score
			THEN 1

--Loss: 0 points
			ELSE 0
		END
	), 0) --Default 0 if the team has no results

	FROM Matches m
	WHERE (m.Team1ID = @TeamID OR m.Team2ID = @TeamID) --Only matches involving this team
		AND m.Team1Score IS NOT NULL --Exclude unplayed matches
		AND m.Team2Score IS NOT NULL
		--Only count group matches, not knockout rounds (Quarter Finals, Semi Finals etc)
		AND (m.TournamentStage = 'Group' OR m.TournamentStage = 'Opening');
...
-- TRIGGER 1: Audit Match Updates
-- Logs score changes to AdminLogs table
CREATE OR ALTER TRIGGER trg_AuditMatchUpdate
ON Matches
AFTER UPDATE 
AS
BEGIN
...
-- TRIGGER 2: Prevent Booking Past Matches
-- Blocks INSERT on Bookings if match date is in the past
CREATE OR ALTER TRIGGER trg_PreventPastMatchBooking
ON Bookings
INSTEAD OF INSERT
AS
BEGIN
    ...
    -- Check if any booking references a past match
    IF EXISTS (
        SELECT 1
        FROM inserted i
    ...
    BEGIN
        RAISERROR('Cannot book tickets for a match that has already been played.', 16, 1);
        RETURN;
    END;
 
    -- If match is in future, allow the insert
...
-- TRIGGER 3: Auto-Log New Player Registrations
-- Fires after a new player is inserted
CREATE OR ALTER TRIGGER trg_LogNewPlayer
ON Players
AFTER INSERT
AS
BEGIN
...
-- TRIGGER 4: Prevent Duplicate Bookings (4-ticket limit)
...
-- TRIGGER 5: Auto-Confirm Booking on Ticket Purchase
...
-- SECURITY TRIGGER 1: Audit Admin Account Updates
CREATE OR ALTER TRIGGER trg_AuditAdminUpdate
ON Admins
AFTER UPDATE
AS
BEGIN
    ...
    SELECT
        @AdminID = d.AdminID,
        @UserName = d.UserName,
        @OldValues = 'UserName: ' + d.UserName
                   + ' | Role: ' + ISNULL(d.AdminRole, 'N/A')
                   + ' | LastLogin: ' + ISNULL(CONVERT(VARCHAR, d.LastLogin, 120), 'Never')
    FROM deleted d;
    ...
    VALUES (
        ...
        'UPDATE',
        'Admins',
        'Admin account updated | Previous values: ' + @OldValues
    );
END;
GO

-- Verification
UPDATE Admins SET AdminRole = 'Operator' WHERE UserName = 'superadmin';
SELECT TOP 1 * FROM AdminLogs ORDER BY LogID DESC;
GO

-- SECURITY TRIGGER 2: Prevent Admin Account Deletion
...
BEGIN
    ...
    VALUES (
        ...
        'DELETE BLOCKED',
        'Admins',
        'Unauthorised deletion attempt on admin account: ' + @UserName
        + ' | Attempted at: ' + CONVERT(VARCHAR, GETDATE(), 120)
    );

    RAISERROR('Admin account deletion is not permitted. The attempt has been logged.', 16, 1);
END;
GO
...
-- SECURITY TRIGGER 3: Log Admin Login Activity
CREATE OR ALTER TRIGGER trg_LogAdminLogin
ON Admins
AFTER UPDATE
AS
BEGIN
    ...
    BEGIN
        SELECT
            @AdminID = i.AdminID,
            @UserName = i.UserName,
            @LoginTime = CONVERT(VARCHAR, i.LastLogin, 120)
        FROM inserted i;
        ...
        VALUES (
            ...
            'LOGIN',
            'Admins',
            'Admin login recorded | User: ' + @UserName + ' | LoginTime: ' + @LoginTime
        );
    END;
END;
GO
...
-- Verification
UPDATE Admins SET LastLogin = GETDATE() WHERE UserName = 'ticket_mgr';
SELECT TOP 1 * FROM AdminLogs ORDER BY LogID DESC;
GO
...
-- CURSOR 1: Match Summary Report with Team Names
-- Iterates through all matches and prints team names with scores
DECLARE @MatchID INT, 
        @Team1ClubName NVARCHAR(100), 
        @Team2ClubName NVARCHAR(100),
        @Team1Score INT, 
        @Team2Score INT, 
        @MatchDate DATETIME, 
        @Stage VARCHAR(50);
...
INTO @MatchID, @Team1ClubName, @Team2ClubName, @Team1Score, @Team2Score, @MatchDate, @Stage;
...
BEGIN
    PRINT 'Match ' + CAST(@MatchID AS VARCHAR) + ' | ' + @Stage 
        ...
        + ' | ' + @Team1ClubName + ' ' + CAST(ISNULL(@Team1Score, 0) AS VARCHAR) 
        + ' - ' + CAST(ISNULL(@Team2Score, 0) AS VARCHAR) + ' ' + @Team2ClubName;
...
-- CURSOR 2: Calculate and Display Total Revenue Per Match
-- Iterates through matches and sums ticket revenue for each
DECLARE @CurMatchID INT, 
        @MatchLabel VARCHAR(200), 
...
WHILE @@FETCH_STATUS = 0
BEGIN
    -- Calculate total ticket revenue for current match
...
-- CURSOR 3: Expired Pending Booking Cancellation
-- Cancels bookings that have been pending for more than 7 days
